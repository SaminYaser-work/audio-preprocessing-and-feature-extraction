...
def _separable_conv(name, kernel, stride, filters, params):
    def _separable_conv_layer(layer_input):
        output = layers.DepthwiseConv2D(name='{}/depthwise_conv'.format(name),
                                        ...
                                        activation=None)(layer_input)
        output = _batch_norm(
            '{}/depthwise_conv/bn'.format(name), params)(output)
        output = layers.ReLU(
        ...
        output = _batch_norm(
            '{}/pointwise_conv/bn'.format(name), params)(output)
        output = layers.ReLU(
...
def yamnet(features, params):
    ...
    for (i, (layer_fun, kernel, stride, filters)) in enumerate(_YAMNET_LAYER_DEFS):
        net = layer_fun('layer{}'.format(i + 1), kernel,
                        stride, filters, params)(net)
...
def yamnet_transfer_image(features, params):
   # net = layers.Reshape(
    ...
    #   input_shape=(params.patch_frames, params.patch_bands))(features)
    for (i, (layer_fun, kernel, stride, filters)) in enumerate(_YAMNET_LAYER_DEFS):
        features = layer_fun('layer{}'.format(i + 1), kernel,
                             stride, filters, params)(features)
    embeddings = layers.GlobalAveragePooling2D()(features)
    return embeddings


def yamnet_frames_model_transfer_image(params, num_class):
    waveform = layers.Input(batch_shape=(
        None, None, None, 3), dtype=tf.float32)
    #waveform = layers.Input(batch_shape=(None,), dtype=tf.float32)
   # waveform_padded = features_lib.pad_waveform(waveform, params)
    # log_mel_spectrogram, features = features_lib.waveform_to_log_mel_spectrogram_patches(
    #   waveform_padded, params)
